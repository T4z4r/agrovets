      ...
      if (query.isEmpty) {
        ...
        _filteredSales = saleProvider.sales.where((sale) {
          return sale.id.toString().contains(query) ||
              (sale.createdAt?.toLowerCase().contains(query.toLowerCase()) ??
                  false);
        }).toList();
      }
    });
    ...
    return Consumer<SaleProvider>(
      builder: (context, saleProvider, child) {
        ...
        if (_filteredSales.isEmpty || _searchQuery.isEmpty) {
          _filteredSales = List.from(saleProvider.sales)
            ..sort((a, b) =>
                (b.createdAt ?? b.saleDate).compareTo(a.createdAt ?? a.saleDate));
        }
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Text(
                                          '${AppLocalizations.of(context)!.saleDate}: ${_formatDate(s.createdAt ?? s.saleDate)}',
                                          style: TextStyle(
                                            color: Colors.grey[600],
                                            fontSize: 12,
                                          ),
          ...
          floatingActionButton: FloatingActionButton(
            onPressed: () async {
              await Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) =>
                      SaleFormScreen(onSave: () => saleProvider.fetchSales()),
                ),
              );
            },
            backgroundColor: Theme.of(context).primaryColor,
